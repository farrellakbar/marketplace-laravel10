                ...
                <div class="col-md-8 col-lg-8">
                    ...
                    <div class="breadcrumb-list">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Management Produk</li>
                        </ol>
                    </div>
                </div>
                            ...
                            <div class="card-header">
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    @can('can_create', [request()])
                                        <button type="button" class="btn btn-sm btn-outline-success" data-bs-toggle="modal" data-bs-target="#modalCenter" onclick="show_create('Create Produk')">Create</button>
                                    @endcan
                                </div>
                            </div>
                            <div class="card-body">
...
<script>
                ...
                columns: [
                    {
                        ...
                        render: function(data, type, row) {
                            if(data.dokumen){
                                return `
                                        <div id='productImage-${data.id}'>
                                            <img src="{{ asset('storage/${data.dokumen.file_path}')}}" onerror="this.onerror=null; this.src='{{ asset('web_assets/images/books/grid/not-found-book.png')}}'" alt="" width="40px;">
                                        </div>
                                        `;
                            }
                            else{
                                return `
                                        <div id='productImage-${data.id}'>
                                            <img src="{{ asset('web_assets/images/books/grid/not-found-book.png')}}" alt="Cover" style="width: 40px;">
                                        </div>
                                        `;
                            }
                        }
                            ...
                            render: function(data, type, row){
                                return `
                                    @can('can_edit', [request()])
                                        <button type="button" class="btn btn-sm btn-outline-warning waves-effect waves-light" data-bs-toggle="modal" data-bs-target="#modalCenter" onclick="show_edit('Edit Produk','${data.id}')">
    ...
    //OPEN MODAL CREATE
        function show_create(title){
            ...
            var act = "{{ route('produk.modal.create')}}";
            $.ajax({
                url: act,
    ...
    //OPEN MODAL UPDATE
        function show_edit(title, param){
            ...
            var act = "{{ route('produk.modal.edit', ['param' => ':param']) }}";
    ...
    //OPEN PROSES DELETE
        function data_delete(id, name) {
            swal({
                ...
                if (result == true) {
                    var act = "{{ route('produk.delete', [':param']) }}";
