        ...
        <div class="row g-3">
            ...
            <div class="col-lg-8 col-md-6 col-sm-12 col-xs-12">
                <div class="form-check form-switch form-switch">
                    <input class="form-check-input" type="checkbox" id="is_active" name="is_active" {{$role->is_active == true ? 'checked' : ''}}>
                  </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    //OPEN PROSES UPDATE
        $('#form_edit').on('submit', function(event) {
            ...
            var act = "{{ route('role.update', [':param']) }}";
                            act = act.replace(':param', '{{encrypt($role->id)}}');
            $.ajax({
                type: "PUT",
                url: act,
                data: data,
