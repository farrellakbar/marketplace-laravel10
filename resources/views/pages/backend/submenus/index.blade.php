        ...
        <div class="breadcrumbbar">
            <div class="row align-items-center">
                <div class="col-md-8 col-lg-8">
                    <h4 class="page-title">Sub Menu {{$menu->name}}</h4>
                    <div class="breadcrumb-list">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item" aria-current="page"><a href="{{route('dashboard')}}">Dashboard</a></li>
                            <li class="breadcrumb-item" aria-current="page"><a href="{{route('menu.index')}}">Management Menu</a></li>
                        ...
                        <div class="card m-b-30">
                            <div class="card-header">
                                <div class="d-flex justify-content-between align-items-center">
                                    <h5 class="card-title">Table Sub Menu</h5>
                                    @can('can_create', [request(),'/sub-menu'])
                                        <button type="button" class="btn btn-sm btn-outline-success" data-bs-toggle="modal" data-bs-target="#modalCenter" onclick="show_create('Create Sub Menu', '{{$param}}')">Create</button>
                                    @endcan
                                </div>
                            </div>
                            <div class="card-body">
                                    ...
                                    <table id="rellarphp-datatable" class="table table-striped table-bordered">
                                        <thead>
                                            <tr>
                                                ...
                                                @if(auth()->user()->can('can_edit', [request(),'/sub-menu']) || auth()->user()->can('can_delete', [request(),'/sub-menu']))
...
<script>
    ...
    //OPEN DATATABLE
        function datatable() {
            var table = $("#rellarphp-datatable").DataTable({
                ...
                scrollX: true,
                ajax: {
                    url: "{{route('submenu.datatable', $param)}}",
                    type: 'GET',
                },
                columns: [
                    {
                    ...
                    @if(auth()->user()->can('can_edit', [request(),'/sub-menu']) || auth()->user()->can('can_delete', [request(),'/sub-menu']))
                        {
                            ...
                            render: function(data, type, row){
                                return `
                                    @can('can_edit', [request(),'/sub-menu'])
                                        <button type="button" class="btn btn-sm btn-outline-warning waves-effect waves-light" data-bs-toggle="modal" data-bs-target="#modalCenter" onclick="show_edit('Edit Sub Menu','${data.id}')">
                                            <i class='fa fa-edit'></i>
                                        </button>
                                    @endcan
                                    @can('can_delete', [request(),'/sub-menu'])
    ...
    //CLOSE DATATABLE
    //OPEN MODAL CREATE
        function show_create(title, param){
            ...
            var act = "{{ route('submenu.modal.create', ['param' => ':param']) }}";
    ...
    //OPEN MODAL UPDATE
        function show_edit(title, param){
            ...
            var act = "{{ route('submenu.modal.edit', ['param' => ':param']) }}";
                act = act.replace(':param', param);
            $.ajax({
                url: act,
                success: function(data){
    ...
    //OPEN PROSES DELETE
        function data_delete(id, name) {
            swal({
                ...
                if (result == true) {
                    var act = "{{ route('submenu.delete', [':param']) }}";
