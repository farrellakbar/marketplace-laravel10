    ...
    <div class="modal-body" id="modalCenterContent">
        <div class="row g-3">
            ...
            <div class="col-lg-8 col-md-6 col-sm-12 col-xs-12">
                <input type="text" class="form-control" name="name" id="name" value="{{$cobaMenu->name}}"
            ...
            <div class="col-lg-8 col-md-6 col-sm-12 col-xs-12">
                <div class="form-check form-switch form-switch">
                    <input class="form-check-input" type="checkbox" id="is_active" name="is_active" {{$cobaMenu->is_active == true ? 'checked' : ''}}>
                  </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    //OPEN PROSES UPDATE
        $('#form_edit').on('submit', function(event) {
            ...
            var act = "{{ route('cobamenu.update', [':param']) }}";
                            act = act.replace(':param', '{{encrypt($cobaMenu->id)}}');
            $.ajax({
                type: "PUT",
                url: act,
                data: data,
